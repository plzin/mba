...
//struct EnumerateLattice<WT: WorkingType> {
//    /// The lattice basis with respect to the Gram-Schmidt basis.
//    r: OwnedMatrix<WT::Scalar>,
//
//    /// The target vector in the Gram-Schmidt basis.
//    qt: OwnedVector<WT::Scalar>,
//
//    /// The radius (L^2 norm) in which all lattice points should be enumerated.
//    rad: WT::Scalar,
//
//    /// The type used for computations.
//    ty: WT,
//
//    /// The coordinates of the point from which the enumeration starts.
//    base_index: OwnedVector<WT::Scalar>,
//
//    /// The indices of the planes.
//    /// The first element is the direction of iteration:
//    /// -1: 0, -1, 1, -2, 2, ...
//    /// 1: 0, 1, -1, 2, -2, ...
//    /// 0: means that the base_index is not yet initialized.
//    offsets: OwnedVector<(usize, isize)>,
//}
//
//impl<WT: WorkingType> EnumerateLattice<WT>
//where WT::Scalar: VectorNorm
//{
//    pub fn new(
//        basis: &IOwnedMatrix, target: &IOwnedVector, rad: WT::Scalar, ty: WT
//    ) -> Self {
//        assert!(basis.ncols() == target.dim(),
//            "Mismatch of basis/target vector dimension.");
//        let bf = basis.transform(|i| ty.from_int(i));
//
//        // Q is the Gram-Schmidt orthonormalization and
...
//        // If the vector not in the span of the basis,
//        // then this will project it into the span.
//        let qt = &q * &target.transform(|i| ty.from_int(i));
//
//        Self {
//            r, qt, rad, ty,
//            base_index: OwnedVector::from_iter(r.nrows(),
//                std::iter::repeat_with(|| ty.zero())
//            ),
//            offsets: OwnedVector::from_iter(r.nrows(),
//                std::iter::repeat((0, 0))
//            ),
//        }
//    }
...
//}
//
//impl<WT: WorkingType> Iterator for &EnumerateLattice<WT> {
//    type Item = IOwnedVector;
//
//    fn next(&mut self) -> Option<Self::Item> {
//        let mut i = 0;
//        while i < self.offsets.dim() {
//            let (dir, o) = &mut self.offsets[i];
//            if *dir != 0 {
//                continue;
//            }
//            let qtc = &self.qt[i];
...
//            let start_index = WT::round(start_index_fl.clone());
//
//            let d = WT::square(start_index.clone() * rc - qtc);
//
//            if d > self.rad {
//                // The plane is not in the radius.
//                // So we can skip it.
//                i -= 1;
//                self.offsets[i] = (0, 0);
//                continue;
//            }
//
//            self.base_index[i] = start_index.clone();
//
//            // We can use a smaller radius inside the plane.
//            // The target to its projection to any point
//            // in the plane form a right triangle.
...
//            // plane can only be <= rad - d.
//            // rad and d are already the square of the distance.
//            let plane_dist = self.rad.clone() - d;
//
//            // Compute the point in the plane we need to be close to now.
//            let point_in_plane = self.qt.clone() - &(&self.r[i] * &start_index);
//
//            // Recursively find the closest point.
//            let Some((mut v, mut w)) = cvp_impl(
//                i - 1, &self.r, &point_in_plane, &plane_dist, self.ty
//            ) else {
//                continue
//            };
//            assert!(v.dim() == i);
...
//            // It is the coordinates of the closest point of
//            // the previous call, concat the index of the plane.
//            v.append(WT::to_int(&start_index));
//
//            // w is the closest point.
//            // It is the closest point of the previous call,
//            // plus the index of the plane times the basis vector
//            // of the current iteration.
//            w += &(&self.r[i][0..i] * &start_index);
//            w.append(start_index.clone() * rc);
//
//            // Compute the distance to the point.
//            let d = (&w - &self.qt[0..i+1]).norm_sqr();
//
//            // If the distance is smaller than the current minimal dist,
//            // then we have found the new best point.
//            if d <= self.rad {
//                self.base_index[i] = start_index_fl;
//            }
//        }
//
//        None

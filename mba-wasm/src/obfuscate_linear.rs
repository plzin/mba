...
pub fn obfuscate_linear(
    expr: String,
    #[wasm_bindgen(js_name = "rewriteOps")]
    rewrite_ops: Vec<String>,
    bits: u32,
    randomize: bool,
    formatter: Formatter,
) -> Result<String, String> {
    mba::choose_binary_ring!(obfuscate_linear_impl(
        expr, rewrite_ops, randomize, formatter, &r
    ), r = bits)
}
...
fn obfuscate_linear_impl<R: BinaryRing>(
    ...
    let rewrite_ops: Vec<_> = rewrite_ops.into_iter().map(|op| {
